...
class CEM:
    def __init__(self, n_squence, elite_ratio, fake_env, upper_bound, lower_bound) -> None:
    ...
    def optimize(self, state, init_mean, init_var):
        ...
        for _ in range(5):
            ...
            constrained_var = np.minimum(np.minimum(np.square(lb_list / 2), np.square(ub_list / 2)), var)
            # generate action squences
...
class PETS:
    ...
    def mpc(self):
        ...
        var = np.tile(
            ...
            self.plan_horizon)
        obs, done, episode_return, steps = self._env.reset()[0], False, 0, 0
